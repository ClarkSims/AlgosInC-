...
public:
    vector<int> prevPermOpt1(vector<int>& A) {
        ...
        if (aoff >= abegin) {
        	...
        	// find maximum element in A[off+1:B.size()] where A[probe] < A[off]
        	int *rhs = &*A.end() - 1;
        	if (*rhs >= a_off) {
        		int *lhs = pmax_off = aoff+1;
        		int *probe = (rhs - lhs)/2 + lhs;
        		for (; probe != lhs && probe != rhs;) {
           			if (*probe < a_off) {
                        if (*probe > *pmax_off) {
                        	pmax_off = probe;
           			    }
        		...
        		// shift left while value is unchanged
        		for (probe = pmax_off - 1; probe >aoff && *probe == *pmax_off; --probe) { }
        		pmax_off = probe;
        	} else {
        		pmax_off = rhs;
        	}

        	// swap B[off] amd B[max_off]
        	int tmp = *aoff;
        	*aoff = *pmax_off;
        	*pmax_off = tmp;
        }
...
bool my_is_same(const vector<int>& lhs, const vector<int>&rhs) {
